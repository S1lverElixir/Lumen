...
на 25 языках. Ответы ИИ не трогаем — модель отвечает на языке собеседника
(см. RESPONSE LANGUAGE в system_prompt.py).

Языки (по алфавиту кода — так пользователю проще искать в меню), 25 штук:
ar, be, bn, de, en (дефолт), es, fa, fil, fr, hi, id, it, kk, ms, nl,
pl, pt, ru, th, tr, uk, ur, uz, vi, zu — топ-30 стран по пользователям
Telegram (сентябрь 2026), все языки этих стран.
...
Первые 6 языков лежат в STRINGS (key -> lang -> текст). Новые языки
добавляются ПАКАМИ (PACK_XX: key -> текст) — так дифф остаётся аддитивным,
не нужно трогать 65 существующих блоков ради каждого языка. t() смотрит
сначала в пак, потом в STRINGS. Тест полноты проверяет оба источника.
