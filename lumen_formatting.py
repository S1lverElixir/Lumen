...
def _convert_markdown_tables_to_lists(text: str) -> str:
    ...
    MarkdownV2) — реальный найденный при тестировании случай: модель генерирует
    '|---|---|', пользователь видит вместо аккуратной таблицы сырую кашу из
    символов "|" построчно. Rich-путь (_md_to_rich_html) шлёт настоящие
    <table> (промпт их разрешает), а это — второй рубеж для обычного HTML-пути:
