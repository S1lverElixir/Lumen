...
- **Automatic model routing** — no `/model` command. Every message is routed to whichever provider actually has what it needs: web search and link reading go to Gemini, everything else defaults to free OpenRouter models to protect Gemini's tight daily quota.
- **Streaming replies** with a self-calibrating typing-speed pacer, so answers appear to type themselves instead of landing in a few large chunks.
- **Prompt-injection and identity-leak defenses** — a deterministic input filter plus output scrubbers keep the bot from revealing which model or provider is actually behind it.
- **TikTok downloader** — videos, slideshows (including "live" photo slides), and original sound, without watermarks.
- **Image generation** via Pollinations.ai, with the model auto-picked from the prompt (anime, fantasy, realism, quick sketch, or a general default).
- **Text-to-speech** — Fish Audio first, Gemini TTS as a fallback chain.
- `/draw` and `/tts` also work as plain phrases at the start of a message ("draw a cat", "read this out loud") — no slash required.
- **Persistent state** — optional Upstash Redis backend, so chat history and quota counters survive redeploys.
- **Error tracking** — optional Sentry integration, with secrets scrubbed before every event is sent.

## How it works

Lumen is a single FastAPI + aiogram service. Telegram delivers updates to a webhook; each message is routed to a chain of candidate models (Gemini and/or OpenRouter) built on the fly from the message's content — attachments, links, and a couple of lightweight heuristics for "does this need current information" and "is this a heavy request." The first provider that answers wins; the other is tried as a fallback if its whole chain fails.

The codebase is a modular monolith: `bot.py` is the orchestrator, and the rest is split into focused modules — `lumen_router_config.py` (model routing), `lumen_formatting.py` (markdown → Telegram HTML), `lumen_security.py` (injection/leak defenses), `lumen_images.py`, `lumen_tts.py`, `lumen_tiktok.py`, `lumen_telegram_transport.py`, `lumen_state_storage.py`, `lumen_typing_pace.py`.
...
All three endpoints below require `Authorization: Bearer <ADMIN_PANEL_KEY>`:
...
`/reset`, `/draw`, and `/tts` show up in Telegram's command menu. A few more exist but stay out of the menu on purpose:
...
- YouTube downloading isn't supported — Hugging Face Spaces' outbound IPs are blocked at the TLS handshake level. Viewing and analyzing a video by link still works.
