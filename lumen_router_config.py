...
_KNOWN_MODEL_IDS_FOR_LEAK_DETECTION: list[str] = [
    ...
    # ДОБАВЛЕНО (аудит моделей, 22 августа 2026, актуальный каталог OpenRouter
...
_OR_MODEL_HEALTH: dict[str, _ModelHealthNote] = {
    ...
    # ── Найдено по реальным логам прода (вечер 17 сентября 2026, уже ПОСЛЕ
    # утреннего аудита — новый код успел задеплоиться и новички пошли в бой) ──
    "thinkingmachines/inkling-small:free": _ModelHealthNote(
        reason="ПОДТВЕРЖДЕНО ПО РЕАЛЬНЫМ ЛОГАМ ПРОДА (17 сентября 2026): 'thinkingmachines/"
                "inkling-small:free is only available on agentic harnesses. Try plugging it "
                "into a coding agent or productivity app' — модель в принципе не обслуживает "
                "обычные chat/completions-запросы, только агентские harness'ы. Стояла второй "
                "в _OR_LIGHT_ORDER — каждое обычное сообщение теряло попытку впустую."
    ),
}
...
# ── Отслеживание анонсированных дат снятия ("Going away <дата>" на карточке
# модели в каталоге OpenRouter) — НЕ то же самое, что _OR_MODEL_HEALTH выше.
# Принцип "не удаляй по спекуляции, только по подтверждённым логам" (см.
# докстринг проекта) означает, что сам факт "у модели на сайте OpenRouter стоит
# будущая дата ухода" — недостаточное основание, чтобы исключить её ИЗ РОУТЕРА
# прямо сейчас, пока она технически ещё жива и отвечает. Но полностью
# игнорировать анонс тоже неверно — тот же класс пробела уже стоил владельцу
# лишнего времени с tencent/hy3:free (истечение промо было замечено только
# постфактум по логам, а не заранее). Этот реестр — золотая середина: просто
# копит известные будущие даты снятия (с каталожной страницы OpenRouter,
# см. историю правок) и громко предупреждает, когда дата наступила — модель
...
def _check_scheduled_removals_due() -> None:
    """Предупреждает, когда анонсированная на карточке OpenRouter дата снятия
...
#
# ПЕРЕСМОТРЕНО ЦЕЛИКОМ (аудит моделей, 22 августа 2026, по реальным логам прода
# HF Spaces той же ночи + актуальному каталогу OpenRouter "Top Weekly free"):
# - inclusionai/ling-3.0-flash:free УБРАНА (была головой) — подтверждённо мертва
#   (см. _OR_MODEL_HEALTH выше, HTTP 404 "use paid slug instead").
# - nvidia/nemotron-3-nano-30b-a3b:free ПОНИЖЕНА ниже прежнего — она унаследовала
#   бы место головы после удаления ling-3.0-flash, но у неё уже 3 подтверждённых
#   калибровкой инцидента порчи текста (см. комментарий в реестре) И анонсирована
#   дата ухода 24 августа 2026 (через 2 дня от последнего аудита, см.
#   _SCHEDULED_OR_REMOVALS) — плохой кандидат на позицию головы прямо перед
#   вероятным исчезновением.
# - nvidia/nemotron-3.5-lightning:free ПОВЫШЕНА до головы: единственный кандидат
#   без единого подтверждённого инцидента (ни порчи текста, ни признаков снятия
#   с тарифа), тот же надёжный nvidia-модельный ряд, что и подтверждённо-хорошие
#   nemotron-3-super/nemotron-3-ultra в _OR_HEAVY_ORDER — до сих пор стояла
...
#
# nemotron-3-super-120b-a12b — ВТОРОЙ инцидент порчи текста (первый: калибровка
# 25 июля 2026, иероглиф вместо "хвост"; второй: прод 17 сентября 2026,
# 'claramente'/'wschod' внутри русских слов — см. комментарий у _OR_HEAVY_ORDER).
# Понижена под ultra-550b, но не исключена: два инцидента за два месяца при
# в целом рабочих ответах — не хроника nano-9b. При третьем — исключать.
#
...
#
# dots-3-note-preview — уже в цепочке (аудит 17 августа), анонсирована к
# снятию 30 сентября 2026 (см. _SCHEDULED_OR_REMOVALS) — не убрана (ещё жива,
# срок далеко), но оставлена ближе к хвосту.
#
# ДОБАВЛЕНО ЦЕЛИКОМ (аудит моделей, 22 августа 2026, по актуальному каталогу
# OpenRouter "Top Weekly free" — см. историю правок): все — coding/agentic-
# модели без диалоговой специализации, ни одна ещё не прогонялась через
# калибровочное сравнение с Claude Sonnet, поэтому расставлены по убыванию
# заявленного размера/возможностей, но целиком после уже проверенных калибровкой
# моделей и перед generic-резервом:
# - z-ai/glm-5.2:free — новая крупная reasoning-модель (1M контекст, сильна в
#   коде и агентных задачах, из той же линейки, что уже подтверждённо мёртвая
#   glm-4.5-air, но это другая, куда более новая модель — нет оснований
#   переносить старый инцидент на неё). ВАЖНО: free-доступ у OpenRouter для
#   платных в остальном моделей этой линейки исторически был временным промо
#   (см. glm-4.5-air) — стоит следить за логами на предмет повторения паттерна.
# - poolside/laguna-s-2.1:free — 118B/8B active, сильный coding-агент
#   (70.2% Terminal-Bench 2.1), уже был в _KNOWN_MODEL_IDS_FOR_LEAK_DETECTION,
#   но ни разу не использовался в реальном роутинге.
# - cohere/north-mini-code:free — 30B/3B active, агентный coding, малый
...
# ОБНОВЛЕНО вечером 17.09.2026 по прод-трафику: nemotron-3-super выдал "кашу"
# из чужих языков прямо в проде (испанское 'claramente', польский фрагмент
# 'wschod' внутри русского слова — тот же класс порчи, что у исключённого
# nano-9b, второй инцидент super после калибровочного 25.07). Не исключён
# (два инцидента за два месяца — не хроническая порча каждого ответа), но
# понижен под ultra-550b, у которого таких инцидентов нет вообще.
